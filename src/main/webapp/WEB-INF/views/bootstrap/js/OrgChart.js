...
OrgChart.prototype.load = function (t) {
	...
	for (c = 0; c < n.length; c++) {
		var O = n[c],
			v = o[O],
			x = i ? i[O] : null;
		if (OrgChart.server._isNotNullEmptyUndefined(v.pid))(P = o[v.pid]) ? ((v.parent = P).children.push(v), P.childrenIds.push(v.id)) : (v.pid = null, v.parent = null, v.isAssistant, l.push(v));
		else v.isAssistant, l.push(v);
	...
	if (null != t.roots) {
		...
		for (c = 0; c < t.roots.length; c++) {
			...
			if (y) {
				if (y.parent)
					for (var b = (P = y.parent).children.length - 1; 0 <= b; b--) {
						(z = P.children[b]) == y && (z.parent = null, P.children.splice(b, 1), P.childrenIds.splice(b, 1))
					}
				y.isAssistant, l.push(y)
			}
		}
	}
	for (c = 0; c < s.length; c++) {
		if (P = o[s[c]]) {
			var w = new OrgChart.node(P.id + "_split_0", P.id, [], "split");
			(o[w.id] = w).isAssistant = !0, (x = i[w.id]) && (o[w.id].state = x.state, o[w.id].state == OrgChart.COLLAPSE && m.push(w.id)), OrgChart.server._initStates(w, o, e, t, r, m);
			var _ = [];
			for (b = P.children.length - 1; 0 <= b; b--) {
				(z = P.children[b]).isAssistant ? (z.parent = null, P.children.splice(b, 1), P.childrenIds.splice(b, 1), _.splice(0, 0, z.id)) : (z.pid = w.id, (z.parent = w).children.push(z), w.childrenIds.push(z.id), P.children.splice(b, 1))
			}
			if (_.length % 2) {
				var k = o[_[_.length - 1]],
					A = [k.w, k.h],
					S = "transperant_" + k.templateName;
				OrgChart.templates[S] = {
					linkAdjuster: {},
					link: "",
					node: "",
					nodeMenuButton: "",
					size: A
				}, (V = new OrgChart.node(P.id + "_transperant", k.pid, [], S)).isAssistant = !0, o[V.id] = V, _.splice(_.length - 1, 0, V.id)
			}
			var I = 1;
			for (b = _.length - 1; 0 <= b; b--) {
				if (b % 2 && b != _.length - 1)(W = new OrgChart.node(P.id + "_split_" + I, null, [], "split")).isAssistant = !0, o[W.id] = W, _.splice(b, 0, W.id), I++;
				else b % 2 && _.splice(b, 0, w.id)
			}
			for (b = 0; b < _.length; b += 3) {
				...
				var M = o[_[b]],
					E = o[_[b + 1]],
					L = o[_[b + 2]];
				M.parent = N, M.pid = N.id, E.parent = N, E.pid = N.id, L.parent = N, L.pid = N.id, N.children.push(M), N.children.push(E), N.children.push(L), N.childrenIds.push(M.id), N.childrenIds.push(E.id), N.childrenIds.push(L.id);
				for (var B = N;
					"split" == B.templateName && (B = B.parent);) B.childrenIds.push(M.id), B.childrenIds.push(E.id), B.childrenIds.push(L.id)
			}
		}
	}
	if (e == OrgChart.action.centerNode) {
		var T = o[r.id];
		T || (T = o[h[r.id]], OrgChart.server._setNodeGroupState(T, OrgChart.EXPAND, t.orientation)), T.state = OrgChart.EXPAND, -1 != (j = m.indexOf(T.id)) && m.splice(j, 1);
		for (var P = OrgChart.server._getRealParent(T), H = T.id; null != P;) {
			if (-1 != (j = m.indexOf(P.id)) && m.splice(j, 1), P.state = OrgChart.EXPAND, r.options.parentState === OrgChart.COLLAPSE_PARENT_NEIGHBORS)
				for (c = 0; c < P.childrenIds.length; c++) {
					var z = o[P.childrenIds[c]];
					if (!OrgChart.server._isSplit(z) && !z.isAssistant && z.id != H) - 1 == (j = m.indexOf(P.childrenIds[c])) && m.push(P.childrenIds[c]), z.state = OrgChart.COLLAPSE
				}
			if (P.state == OrgChart.EXPAND)
				for (var U = 0; U < P.childrenIds.length; U++) {
					var F = o[P.childrenIds[U]];
					if (OrgChart.server._isSplit(F) || F.isAssistant) - 1 != (j = m.indexOf(F.id)) && m.splice(j, 1), F.state = OrgChart.EXPAND
				}
			H = P.id, P = OrgChart.server._getRealParent(P)
		}
		if (r.options.childrenState === OrgChart.COLLAPSE_SUB_CHILDRENS)
			for (c = 0; c < T.childrenIds.length; c++) {
				if (-1 != (j = m.indexOf(T.childrenIds[c])) && m.splice(j, 1), (z = o[T.childrenIds[c]]).state = OrgChart.EXPAND, !z.isAssistant && !OrgChart.server._isSplit(z))
	...
	for (c = 0; c < m.length; c++) {
		if (P = (v = o[m[c]]).parent)
			for (b = P.children.length - 1; 0 <= b; b--) {
				if (P.children[b] == v)
					if (-1 == P.collapsedChildrenIds.indexOf(v.id) && P.collapsedChildrenIds.push(v.id), P.children.splice(b, 1), "split" == v.templateName || "split" == P.templateName) {
						if (N = v.parent) {
							for (;
								"split" == N.templateName;) N = N.parent;
							for (var G = 0; G < N.childrenIds.length; G++) {
								var X = N.childrenIds[G]; - 1 == N.collapsedChildrenIds.indexOf(X) && N.collapsedChildrenIds.push(X)
							}
						}
					} else v.parent = null
			}
	...
	if (t.layout == OrgChart.mixed)
		for (c = 0; c < n.length; c++) {
			if (0 == (v = o[n[c]]).childrenIds.length && !v.isAssistant && "split" != v.templateName)
				if (P = v.parent) {
					if (!OrgChart.server._mixedLayoutAllNodes(P)) continue;
					for (b = P.children.length - 1; 0 <= b; b--) {
						if ((z = P.children[b]) != v && 0 == z.childrenIds.length && !z.isAssistant && "split" != z.templateName) {
							z.parent = v, z.pid = v.id, z.layout = OrgChart.mixed, P.children.splice(b, 1);
	...
	if (t.layout == OrgChart.tree || t.layout == OrgChart.treeRightOffset || t.layout == OrgChart.treeLeftOffset) {
		var Y = [];
		for (c = 0; c < n.length; c++) {
			if (0 == (v = o[n[c]]).childrenIds.length && !v.isAssistant && "split" != v.templateName)(P = v.parent) && -1 == Y.indexOf(P.id) && Y.push(P.id)
		}
		for (c = 0; c < Y.length; c++) {
			P = o[Y[c]], w = new OrgChart.node(P.id + "_split_0", P.id, [], "split");
			(o[w.id] = w).layout = OrgChart.tree;
			var q = [];
			for (b = P.children.length - 1; 0 <= b; b--) {
				if (OrgChart.server._mixedLayoutAllNodes(P))
					if (0 == (z = P.children[b]).childrenIds.length && !z.isAssistant && "split" != z.templateName && (z.parent && "split" != z.parent.templateName || z.parent.isAssistant)) {
						if ((z = P.children[b]).parent = null, z.layout = OrgChart.tree, P.children.splice(b, 1), P.childrenIds.splice(b, 1), t.layout == OrgChart.treeRightOffset && q.splice(0, 0, z.id), t.layout == OrgChart.treeLeftOffset || t.layout == OrgChart.treeRightOffset) {
							var V;
							S = "transperant_" + z.templateName;
							OrgChart.templates[S] = {
								linkAdjuster: {},
								link: "",
								node: "",
								nodeMenuButton: "",
								size: [0, 0]
							}, (V = new OrgChart.node(z.id + "_transperant", null, [], S)).layout = OrgChart.tree, o[V.id] = V, q.splice(0, 0, V.id)
						}
						t.layout != OrgChart.treeRightOffset && q.splice(0, 0, z.id)
					}
			}
			for (I = 1, b = q.length - 1; 0 <= b; b--) {
				var W;
				if (b % 2 && b != q.length - 1)(W = new OrgChart.node(P.id + "_split_" + I, null, [], "split")).layout = OrgChart.tree, o[W.id] = W, q.splice(b, 0, W.id), I++;
				else b % 2 && q.splice(b, 0, w.id)
			}
			for (b = 0; b < q.length; b += 3) {
				N = null;
				0 == b && (N = P);
				M = o[q[b]], E = o[q[b + 1]], L = o[q[b + 2]];
				0 != b && (N = o[q[b - 3]]), 0 == b || E || (N = o[q[b - 2]]), M.parent = N, M.pid = N.id, N.children.push(M), P.childrenIds.push(M.id), E && (0 != b && (N = o[q[b - 2]]), E.parent = N, E.pid = N.id, N.children.push(E), P.childrenIds.push(E.id)), L && (0 != b && (N = o[q[b - 1]]), L.parent = N, L.pid = N.id, N.children.push(L), P.childrenIds.push(L.id))
			}
		}
	}
	return {
	...
	if (this.config.showYScroll === OrgChart.scroll.visible || this.config.mouseScrool === OrgChart.action.scroll || this.config.mouseScrool === OrgChart.action.yScroll) {
		var a = this;
		this.bar && this.bar.parentNode.removeChild(this.bar), this.bar = document.createElement("div"), this.config.showYScroll !== OrgChart.scroll.visible && (this.bar.style.visibility = "hidden"), this.innerBar = document.createElement("div"), this.innerBar.innerHTML = " ", Object.assign(this.bar.style, {
			position: "absolute",
			right: 0,
			bottom: 0,
			height: t + "px",
